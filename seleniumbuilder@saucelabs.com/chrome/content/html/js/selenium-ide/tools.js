...
Log.prototype = {
    _getThreshold: function() {
        if (!this.threshold) {
            this.threshold = SeleniumIDE.Preferences.getString("internalLogThreshold", "INFO");
        }
...
function showFilePicker(window, title, mode, defaultDirPrefName, handler, defaultFileName) {
  ...
  var defaultDir = SeleniumIDE.Preferences.getString(defaultDirPrefName);
  if (defaultDir) {
  ...
  if (res == nsIFilePicker.returnOK || res == nsIFilePicker.returnReplace) {
    SeleniumIDE.Preferences.setString(defaultDirPrefName, fp.file.parent.path);
    return handler(fp);
  } else {
    return null;
  }
